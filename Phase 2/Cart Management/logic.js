...
function displayData() {
    var tableContents = '';
    var startTable = '<table class="table table-striped"><thead><tr><th scope="col">Item Name</th><th scope="col">Price</th><th scope="col">Quantity</th></tr></thead><tbody>';
    var endTable = '</tbody></table><br><br><br>';
    var totalCost = 0;
    for (var i = 0; i < sessionStorage.length; ++i) {
        if (sessionStorage.key(i) != 'IsThisFirstTime_Log_From_LiveServer') {
            var tempJson = JSON.parse(sessionStorage.getItem(sessionStorage.key(i)));
